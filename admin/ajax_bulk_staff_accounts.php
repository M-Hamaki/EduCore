...
require_once '../src/Modules/Staff/StaffAccountBulkCommandService.php';
...
try {
    ...
    // Get list of active staff roles
    $action = (string)($_POST['action'] ?? '');
    $onError = (string)($_POST['on_error'] ?? 'stop');
    $roleStmt = $db->query("SELECT role_key FROM staff_roles WHERE status = 'active'");
    $validRoles = array_values(array_unique(array_merge(
        ['employee', 'teacher', 'specialist', 'doctor', 'librarian'],
        array_filter(
            array_map('strval', $roleStmt->fetchAll(PDO::FETCH_COLUMN) ?: []),
            static fn(string $roleKey): bool => !in_array($roleKey, ['admin', 'super_admin'], true)
        ),
        $activeRole === 'super_admin' ? ['admin', 'super_admin'] : []
    )));

    $selection = \EduCore\Modules\Accounts\AccountBulkSelection::fromArray($_POST);
    $service = new \EduCore\Modules\Staff\StaffAccountBulkCommandService($db);
    ...
    if (!$isClientError) {
        error_log('Staff bulk account action failed: ' . $e->getMessage());
    }
