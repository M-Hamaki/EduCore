...
if (array_key_exists('help', $options)) {
    echo "Usage: php tools/staff_discipline_finance_effect_dispatcher.php [--apply --database=<exact_database> --actor-id=<service_actor_id> --gateway-bootstrap=<project_relative_php_file>] [--limit=50] [--json]\n";
    echo "Without --apply the command only lists due Staff discipline Finance effect IDs and never contacts Finance.\n";
    exit(0);
}
...
try {
    ...
    $repository = new PdoDisciplineFinanceEffectRepository($db);
    $dueIds = $repository->dueEffectIdsForDispatch($limit, gmdate('Y-m-d H:i:s.u'));

    if (!$apply) {
        staffDisciplineFinanceDispatchOutput([
            'mode' => 'dry-run',
    ...
    $gateway = staffDisciplineFinanceGateway($root, $gatewayBootstrap, $db);
    $result = (new StaffModuleFactory($db, new AuditService($db)))
        ->disciplineFinanceEffects($gateway)
        ->dispatchDueEffects($limit, (int) $actorText);
    staffDisciplineFinanceDispatchOutput([
        'mode' => 'apply',
        'connected_database' => $connectedDatabase,
        'limit' => $limit,
    ...
    error_log('staff discipline Finance effect dispatcher failed: ' . $exception->getMessage());
    fwrite(STDERR, "DISPATCH_FAILED=the operation was not completed; inspect the protected server log.\n");
    exit(1);
}

/**
 * Finance owns the bootstrap that returns its narrow fact gateway. Only a
 * project-relative file is accepted, so a CLI user cannot include arbitrary
 * absolute paths or escape the repository root.
 */
function staffDisciplineFinanceGateway(string $root, string $bootstrap, PDO $db): PayrollImpactGateway
{
    if (preg_match('/^(?:[A-Za-z]:|[\\\\\\/])/', $bootstrap) === 1) {
        throw new InvalidArgumentException('DISCIPLINE_FINANCE_GATEWAY_BOOTSTRAP_INVALID');
    }
    ...
    if (!$gateway instanceof PayrollImpactGateway) {
        throw new InvalidArgumentException('DISCIPLINE_FINANCE_GATEWAY_CONTRACT_INVALID');
    }
...
function staffDisciplineFinanceDispatchOutput(array $report, bool $json): void
{
    if ($json) {
